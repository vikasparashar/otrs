...
@body;
$query2 = "select tsk.ticket_id,tsk.task,cust.name from Task_list tsk ,customer_company cust where tsk.customer_name=cust.customer_id";
...
while ( @row2=$sth2->fetchrow_array() )
{
      ...
      open (MYFILE, ">>$file");
      print MYFILE "<tr><td>$row2[2]</td><td>onsite</td><td>$row2[1]</td><td>Pending</tr>";
      close (MYFILE);
}
